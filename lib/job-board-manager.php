...
function job_board_manager_get_page_content_list($container_guid = NULL) {
	...
	elgg_register_title_button('job-board-manager', 'add', 'object', 'job-board-manager');
...
function job_board_get_page_content_edit($page, $guid = 0, $revision = NULL) {
	...
	if ($page == 'edit') {
		...
		if (elgg_instanceof($jobBoardManager, 'object', 'job-board-manager') && $jobBoardManager->canEdit()) {
			...
			$content = elgg_view_form('job-board-manager/save', $vars, $body_vars);
			$sidebar = elgg_view('job-board-manager/sidebar/revisions', $vars);
		} else {
